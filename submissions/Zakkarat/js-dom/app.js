...
let colorBody = ["jhin-body", "mf-body", "caitlyn-body", "kaisa-body"];
let colorHeader = ["jhin-header", "mf-header", "caitlyn-header", "kaisa-header"];
let colorDesk = ["jhin-desk-head", "mf-desk-head", "cait-desk-head", "kaisa-desk-head"];
...
function champAuto(a) {
  for (let i = 0; i < champ.length / 2; i++) {
    if (champ[i] == a || champ[i + 4] == a){
      ...
      head.classList.add(colorHeader[i]);
      desk.classList.add(colorDesk[i]);
      cont.classList.add(colorHeader[i]);
      document.body.classList.add(colorBody[i]);
...
function clearify() {
    let old = [document.body.classList, topHead.classList, desk.classList, cont.classList];
    old.forEach(el => el.remove(el[el.length - 1]))
}
 function menu() {
