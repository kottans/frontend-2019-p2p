...
class Inhabitant {
    constructor (name, gender, says, species) {
        ...
        this.says = says;
        this.friend = "no friends yet :( "
    }

    toString(){
        let inhabitantProfile = [this.name, this.species, this.gender, this.says, `friends: ${this.friend}`, ""];
        return inhabitantProfile.join(", ");
    }

    makeFriendWith (inhabitant) {
        this.friend = inhabitant.name;
        inhabitant.friend = this.name;
    }
}
